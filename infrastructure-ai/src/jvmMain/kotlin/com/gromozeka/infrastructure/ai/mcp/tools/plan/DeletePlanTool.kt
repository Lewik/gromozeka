...
import com.gromozeka.domain.service.plan.DeletePlanTool as DeletePlanToolInterface
...
class DeletePlanTool(
...
) : GromozekaMcpTool, DeletePlanToolInterface {
    ...
    override val definition = Tool(
        name = "delete_plan",
        description = "Delete plan with all its steps (cascades to child steps)",
        inputSchema = Tool.Input(
            properties = buildJsonObject {
                put("planId", buildJsonObject {
                    put("type", "string")
                    put("description", "Plan ID to delete")
                })
            },
    ...
    override suspend fun execute(planId: String): Map<String, Any> {
        val id = Plan.Id(planId)
        planManagementService.deletePlan(id)
        
        
        return mapOf(
            "success" to true,
            "message" to "Plan $planId deleted successfully"
        )
    }
}
