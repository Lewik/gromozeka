...
You are an AI agent tasked with analyzing a screenshot and generating a color theme that **uses the exact same colors as
the captured content**. The primary goal is color matching - the user wants their application to use the same colors
they see in the screenshot.
...
- **themeName**: Use descriptive name based on dominant colors or image content (e.g., `"Ocean Sunset"`,
  `"Forest Green"`, `"Corporate Blue"`)
...
**Important**: Focus on the **structure and format** of the JSON, not the specific color values. Your task is to
generate completely new colors based on the target screenshot while following the same structural pattern.
