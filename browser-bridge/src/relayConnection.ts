...
export function debugLog(...args: unknown[]): void {
  ...
  if (enabled) {
    ...
    console.log('[Gromozeka Browser Bridge]', ...args);
  }
}
...
export class RelayConnection {
  ...
  // Simulates a "new tab opened" event; the relay responds by calling
  ...
  private async _handleCommand(message: ProtocolCommand): Promise<any> {
    ...
    const args = browserBridgeCommandArguments(message.method, message.params);
...
export function browserBridgeCommandArguments(method: string, params: unknown): any[] {
  const args = Array.isArray(params) ? params : [];
  if (method !== 'chrome.tabs.create')
    return args;

  const requestedOptions = args[0] && typeof args[0] === 'object' ? args[0] : {};
  return [{ ...requestedOptions, active: false }];
}
