...
class ClaudeMessageTypesTest : FunSpec({
    ...
    test("analyze message field structures in today's files") {
                                ...
                                entry.message?.let { messageElement ->
                                    ...
                                    when (messageElement) {
                                        is JsonObject -> {
                                            ...
                                            try {
                                                val typedMessage = json.decodeFromString<ClaudeLogEntry.Message>(messageElement.toString())
                                                // Successfully parsed
                                            } catch (e: Exception) {
        ...
        println("\nParse success rate: ${((messageFieldCount - parseErrors.size) * 100.0 / messageFieldCount.coerceAtLeast(1)).toInt()}%")
    }
                    ...
                    try {
                        ...
                        messageElement?.let { element ->
                            ...
                            try {
                                when (element) {
                                    is JsonObject -> {
                                        val typedMessage = json.decodeFromString<ClaudeLogEntry.Message>(element.toString())
                                ...
                                if (failureExamples.size < 5) {
                                    failureExamples.add("Parse failure: ${e.message} | JSON: ${element.toString().take(200)}")
                                }
                            }
                        }
