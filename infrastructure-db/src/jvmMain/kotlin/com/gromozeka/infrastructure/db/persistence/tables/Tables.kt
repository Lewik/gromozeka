...
internal object AgentSkills : Table("agent_skills") {
    ...
    val materializationPolicy = varchar("materialization_policy", 32)
    val materializationReason = text("materialization_reason")
    val materializationModelConfigurationId = varchar("materialization_model_configuration_id", 255).nullable()
    val materializationAnalyzedAt = timestamp("materialization_analyzed_at").nullable()
...
internal object TokenUsageStatisticsTable : Table("ai_usage_records") {
    val id = varchar("id", 255)
    val userId = varchar("user_id", 255).nullable()
    val projectId = varchar("project_id", 255).nullable()
    val agentDefinitionId = varchar("agent_definition_id", 255).nullable()
    val conversationId = varchar("conversation_id", 255).nullable()
    val threadId = varchar("thread_id", 255).nullable()
    val lastMessageId = varchar("last_message_id", 255).nullable()
    val timestamp = timestamp("timestamp")
    val runtimePurpose = varchar("runtime_purpose", 128)
    val executionTarget = varchar("execution_target", 255)
    val connectionKind = varchar("connection_kind", 64)
    val connectionId = varchar("connection_id", 255)
    val modelConfigurationId = varchar("model_configuration_id", 255)
    ...
    val provider = varchar("provider", 50)
    val modelId = varchar("model_id", 255)
    val contextInputTokens = integer("context_input_tokens").nullable()
    val pricingCatalogVersion = varchar("pricing_catalog_version", 128).nullable()
    val pricingEffectiveAt = timestamp("pricing_effective_at").nullable()
    val inputNanoUsdPerMillion = long("input_nano_usd_per_million").nullable()
    val cacheCreationNanoUsdPerMillion = long("cache_creation_nano_usd_per_million").nullable()
    val cacheReadNanoUsdPerMillion = long("cache_read_nano_usd_per_million").nullable()
    val outputNanoUsdPerMillion = long("output_nano_usd_per_million").nullable()
    val estimatedCostNanoUsd = long("estimated_cost_nano_usd").nullable()
