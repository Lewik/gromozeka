...
class LlmMemoryClaimExtractor(
    ...
    private companion object {
        val CLAIM_EXTRACTOR_SYSTEM_PROMPT = """
            ...
            - For assistant-produced tables, schedules, checklists, matrices, or final drafted artifacts inside imported conversation/source text, the assistant output is valid imported evidence when the source is being ingested as a past chat/document. Extract exact cell-level assignment facts when the artifact contains concrete named people/items and concrete slots. Prefer predicate "assigned_to_shift" for person/agent schedule cells that assign someone to a day, time range, route, or rotation slot.
