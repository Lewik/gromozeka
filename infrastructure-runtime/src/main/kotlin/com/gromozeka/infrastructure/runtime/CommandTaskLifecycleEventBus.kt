...
class InMemoryCommandTaskLifecycleEventBus :
    ...
    private class InMemoryDelivery(
        override val event: CommandTaskLifecycleEvent,
    ) : CommandTaskLifecycleEventDelivery {
...
internal class RabbitCommandTaskLifecycleEventPublisher(
    private val rabbitTemplate: RabbitTemplate,
    private val topology: RabbitCommandTaskLifecycleEventTopology,
) : CommandTaskLifecycleEventPublisher {
    private val json = Json { encodeDefaults = true; ignoreUnknownKeys = false }

    init {
        rabbitTemplate.requireCommandTaskEventPublisherConfirms()
    }

    override suspend fun publish(event: CommandTaskLifecycleEvent) {
        topology.declare()
        rabbitTemplate.publishCommandTaskEventConfirmed { operations ->
...
internal class RabbitCommandTaskLifecycleEventConsumer(
    private val connectionFactory: ConnectionFactory,
    private val topology: RabbitCommandTaskLifecycleEventTopology,
) : CommandTaskLifecycleEventConsumer, SmartLifecycle {
        ...
        listenerContainer = SimpleMessageListenerContainer(connectionFactory).apply {
            ...
            setMessageListener(ChannelAwareMessageListener { message, rabbitChannel ->
                ...
                val delivery = try {
                    ...
                    log.error(error) { "Rejected invalid command lifecycle event: ${error.message}" }
                ...
                try {
                    ...
                    log.error(error) { "Command lifecycle event settlement failed: ${error.message}" }
    ...
    private class RabbitDelivery(
        ...
        private suspend fun settle(value: DeliveryDecision) {
            check(decision.complete(value)) {
                "Command lifecycle event was already settled: ${event.taskId.value}"
            }
            settlement.await()
        }
    }
...
@Service
@ConditionalOnProperty(name = ["gromozeka.runtime.rabbit.enabled"], havingValue = "true")
internal class RabbitCommandTaskLifecycleEventTopology(
    private val connectionFactory: ConnectionFactory,
    @Value("\${gromozeka.runtime.rabbit.command-events-exchange:gromozeka.command.events}")
    val exchangeName: String,
    @Value("\${gromozeka.runtime.rabbit.command-events-queue:gromozeka.command.events.server}")
    val queueName: String,
    @Value("\${gromozeka.runtime.rabbit.command-events-routing-key:command-task}")
    ...
    fun declare() {
        ...
        repeat(TOPOLOGY_DECLARE_ATTEMPTS) { attempt ->
            try {
                ...
                log.warn(error) {
                    "Rabbit command event topology is not ready: " +
...
private fun RabbitTemplate.requireCommandTaskEventPublisherConfirms() {
    val factory = connectionFactory
    check(factory.isPublisherConfirms || factory.isSimplePublisherConfirms) {
        "Rabbit command lifecycle event delivery requires publisher confirms"
    }
}

private fun RabbitTemplate.publishCommandTaskEventConfirmed(
